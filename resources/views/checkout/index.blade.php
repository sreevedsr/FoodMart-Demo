  ...
  <div class="container mt-5">
    ...
    @else
      ...
      <div class="card p-3 mb-3">
        ...
        <p>{{ $defaultAddress->address_line }}, {{ $defaultAddress->city }}, {{ $defaultAddress->state }} - {{ $defaultAddress->pincode }}</p>
        <button class="btn btn-sm btn-link text-decoration-none text-primary" data-bs-toggle="modal" data-bs-target="#changeAddressModal">
          Change
        </button>
      </div>
                    ...
                    <div class="modal-content">
                      <form action="{{ route('checkout.address.edit', $address) }}" method="POST">
                        ...
                        <div class="modal-body">
                          ...
                          <input type="text" name="address_line" value="{{ $address->address_line }}" class="form-control mb-2" required>
      ...
      <form action="{{ route('checkout.placeOrder') }}" method="POST" id="placeOrderForm">
        ...
        <button class="btn btn-purple mt-3" type="submit">
          Place Order
        </button>
      </form>
    @endif
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    addressCards.forEach(card => {
      card.addEventListener("click", function (e) {
        // Skip clicks on the edit button
        if (e.target.closest("button")) return;

        const id = card.getAttribute("data-id");
        hiddenInput.value = id;

        // Update visible default address
        const defaultAddressDiv = document.querySelector(".card.p-3.mb-3");
        defaultAddressDiv.innerHTML = `
          ${card.innerHTML}
